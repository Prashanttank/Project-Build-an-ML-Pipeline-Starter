...
def go(args):
    ...
    # Load training and validation data artifact
    trainval_local_path = run.use_artifact(args.trainval_artifact).file()

    X = pd.read_csv(trainval_local_path)
    y = X.pop("price")  # remove "price" column from X and store it in y
    ...
    # Compute R-squared and Mean Absolute Error (MAE) on validation data
    logger.info("Scoring")
    ...
    mlflow.sklearn.save_model(sk_pipe, "random_forest_dir", signature=signature, input_example=X_train.iloc[:5])
    ######################################
    ...
    ######################################
    # Log R-squared and MAE metrics to W&B
...
def get_inference_pipeline(rf_config, max_tfidf_features):
    ...
    # Pipeline for non-ordinal categorical features
    non_ordinal_categorical_preproc = make_pipeline(
        SimpleImputer(strategy="most_frequent"),
        OneHotEncoder()
    )

    # Impute numerical columns with zeroes
    zero_imputed = [
        "minimum_nights",
        "number_of_reviews",
        "reviews_per_month",
